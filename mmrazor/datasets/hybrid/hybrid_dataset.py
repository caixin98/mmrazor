...
from mmdet.datasets import build_dataset as build_det_dataset
...
class HybridDataset(Dataset):
    def __init__(self,
        ...
        if det_dataset is not None:
            self.det_dataset = build_det_dataset(det_dataset)
            length = max(length, self.det_dataset.__len__())
            num_datasets += 1
    ...
    def __getitem__(self, index):
        if not self.test_mode:
            ...
            if hasattr(self, 'det_dataset'):
                index_det = np.random.randint(low=0, high=self.det_dataset.__len__())
                input_dict['det'] = self.det_dataset.__getitem__(index_det)
            ...
            if hasattr(self, 'det_dataset'):
                if index < self.det_dataset.__len__():
                    input_dict['det'] = self.det_dataset.__getitem__(index)
            if hasattr(self, 'pose_dataset'):
    ...
    def evaluate(self,
        ...
        for res in results:
            ...
            if 'det_result' in res.keys():
                det_results.append(res['det_result'])
        ...
        if hasattr(self, 'det_dataset'):
            #print(det_results)
            eval_results['det_result'] = self.det_dataset.evaluate(det_results, logger=logger, **det_args)
        if hasattr(self, 'pose_dataset'):
