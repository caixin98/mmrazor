...
from mmrazor.models.builder import HYBRIDS, build_posenet, build_detector, build_classifier, build_optical
    ...
    def forward_train(self, input_dict):
        ...
        if random.random() < 0.5:
            ...
            if hasattr(self, 'detector'):
                ...
                data_copy = data.copy()
                data_copy["img"] = img
                losses = self.detector(**data_copy, return_loss=True)
                for key in losses.keys():
    ...
    def forward_test(self, input_dict):
        ...
        if 'det' in input_dict.keys():
            ...
            if isinstance(img, list):
                img = [self.forward_optical(tmp) for tmp in img]
                data['img'] = img
            else:
                data['img'] = self.forward_optical(img)
